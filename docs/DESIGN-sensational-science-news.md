...
## 22. Rev 7 amendment — the "Full" asset metadata model (owner decision at the A7 gate)

The owner chose FULL (2026-09-26): the Library tab's approved mockup — MULTIPLE Mood/Energy tags per asset and an editable Type + rename — is honoured end to end, not the single-value backend of rev 3. This reworks the owner-asset metadata model (re-opens the A3/A5/A6 row + endpoints and adds SDK support); the grants model (A2/A4) and deactivate (A1) are unchanged.

Model:
- MOOD/ENERGY are MULTI-VALUE, stored as the asset's `tags` tuple (the library's existing multi-value field) with the prefixes `mood:` and `energy:` (e.g. `tags = ("mood:ominous", "mood:mysterious", "energy:building")`). This keeps the two dimensions distinct within the flat tag set and lets the workflow later select a track with `ctx.library.find(tags=["mood:ominous"])`. The generic single-valued `facets` field is dropped from the owner-asset ROW; the asset row becomes `{id, name, kind, status, mood: string[], energy: string[], description, grant}` (mood/energy are the prefix-stripped tag values).
- RENAME: the asset's display name is its alias; changing it updates `aliases.json` (remove the old name -> id, add the new). New SDK `LibraryStore.rename(asset_id, name)`.
- RE-KIND: change the asset's `kind` (music/sfx/voice) — a sidecar rewrite (like the status flip). New SDK support to set kind, and to replace `tags`, on an existing asset (a unified descriptor update, e.g. extend `annotate` to also take `kind`/`tags`, or add `set_kind`/`set_tags`).
- ENDPOINTS: upload (`POST /assets`) accepts `name`, `kind`, `grant`, and `mood`/`energy` as JSON arrays. Edit (`PUT /assets/{id}`) accepts optional `name`, `kind`, `mood[]`, `energy[]`, `caveats` and applies rename + re-kind + tag-replace + caveats. The row (GET/upload/edit) returns mood/energy as arrays + name + kind.
- ACCESSIBILITY (A7 design-auditor BLOCKING): each Mood/Energy chip is a focusable element with a keyboard-invocable EDIT (Enter/F2) and a real delete control (a `<button>` "x" with an accessible name); the right-click Edit/Delete menu is KEPT (owner-requested) but is now an addition, and is Escape-dismissible with focus moved into it. The mockup is amended to show the chip `x` + keyboard affordance (a reasonable accessibility improvement to the approved design, recorded in CHANGES, no re-approval needed).
- The single-value facet validation vocabulary (mood/energy FacetSpec) from rev 3 is replaced by the prefixed-tag model; the workflow's music/SFX/voice selection (§3.6, §6.5) reads these via tags. — grant security posture (A4 security-auditor)

The grant system is an OWNER-FACING MANAGEMENT / CURATION boundary enforced at the `ctx.library` SDK API layer (find/get/path all grant-check every owner-pool access; the owner pool is read-only to workflows; a workflow cannot widen its own access through the API). It is NOT an OS-level sandbox against a hostile workflow: the owner pool `library/_owner` is a filesystem SIBLING of a workflow's own namespace (`library/<id>`), so a workflow with filesystem access to the data dir can derive and read the pool (or rewrite `grants.json`) directly, whether or not the path is serialized into `context.json`. This is acceptable and by design because SFVF is a SINGLE-USER LOCAL app in which THE OWNER AUTHORS/APPROVES EVERY WORKFLOW — there is no adversarial-workflow threat in scope; grants exist so the owner can curate which of their workflows draw on which assets, not to defend against their own code. A hard sandbox (OS confinement of the workflow subprocess) is explicitly out of scope for this cycle. The WORKFLOW-WIRING increment (when the supervisor sets `ctx.paths.library_owner_pool` for a run) carries this posture; it needs no extra hardening beyond the API-layer grant checks. VALUE-ASSET ASYMMETRY (security-auditor ADVISORY-2): `ctx.library.value()`/`describe()` do not consult the owner pool — intentional and fail-closed, because owner-pool assets are FILE assets (music/SFX/voice) read via get/path/find; `value()` serves the workflow's own value-kind assets (series state). Not a defect.
