...
with the video path made **relative to the video folder** (SDK §5.5) — so an example workflow's finished
file reaches `video.json` by returning it, rather than hand-emitting a result event. Workflows that return
`None` and emit their own event are unaffected (backward compatible).
