...
## Installer — npm ci runs unconditionally; a failed install is now capturable (change cycle, PR #169)
The installer (`install.cmd`/`install.ps1`) now runs `npm ci` every time before building the frontend, instead of skipping it whenever a `frontend/node_modules` already existed — a stale or incomplete `node_modules` (missing the dev-dependencies the TypeScript build compiles against) previously failed the build at "Building the frontend"; `npm ci` reinstalls exactly from the lockfile, so the build always has what it needs. A failed `install.cmd` also no longer vanishes: the whole run is transcribed to `install_report.txt` beside the installer and the console window stays open (`pause`), so an install error can be read and handed over. Installer robustness only — the installed SFVF application is byte-for-byte unchanged, so VERSION deliberately stays 1.0.0 (there is no shipped-app change to version; the deliverable is re-verified by `scripts/install-check.ps1`).

## Library subsystem — an asset library with per-asset access control (Sensational Science News change cycle, Stage A, PR #170)
SFVF gains a **Library** tab and the storage behind it, so the owner can hand-select reusable assets (soundtracks, SFX, and — later — voices) that outlive individual runs. Assets are content-addressed and never erased: "remove" **deactivates** (hides from selection) rather than deletes, and content history is preserved. Each asset carries **access control**: it can be granted to **all** workflows or to a chosen **set of specific** workflows, editable per asset. The tab lists every asset (active and hidden) with a detail editor — a friendly name (rename after upload), a **Type** (music/sfx/voice, changeable), multi-value **Mood** and **Energy** tags (comma-to-chip, right-click or keyboard edit/delete, fully keyboard- and screen-reader-operable), free-text notes, the access picker, and upload (including a voice clip for future voice cloning). Under the hood: a namespaced content-addressed `LibraryStore` with deactivate/reactivate/rename/re-kind; a fail-closed `GrantStore` (a missing or corrupt grant denies access); app endpoints for listing, upload (25 MiB streaming cap, audio-type/kind validation), edit, grant, and deactivate; and a `ctx.library` facade that lets a workflow read its **own** namespace **plus** the owner-pool assets granted to it (grant-filtered `find`/`get`/`path`), with the owner pool read-only to workflows. A workflow only ever sees an owner asset that is both **granted to it** and **active** (a deactivated asset is withheld even by id), and a workflow read never creates the owner pool on disk. No workflow consumes the library yet — that lands in later stages of this change cycle.

## Workflow enablers — run settings, per-video budget cap, audio mixing, voices, descriptions (Sensational Science News change cycle, Stage B)
The chassis gains the pieces a real generation workflow needs, all still behind existing surfaces. **Run settings at launch:** the launch form now offers an **Approval mode** (Manual — approve before the money-spending steps — vs Autonomous), a **Per-video budget** cap, and a **Voice** picker; all three travel into the run and are validated at the boundary (a non-positive/non-finite budget or an unsafe voice id is rejected). **Per-video budget ceiling:** the budget guard now caps the *total* spend of one video across *all* providers (not just per-provider), refusing the call that would breach the chosen per-video budget, while preserving the existing per-provider/per-day caps and the reserve/release-on-failure behaviour (a failed paid call frees its reservation). **Audio mixing:** a new `media.edit.mix` combines a narration track with an optional music bed — **ducked** under the voice — and offset sound effects into a single track. **Selectable voices with cloning:** `media.speech` now speaks in a chosen voice — a bundled public-domain preset or one of the owner's uploaded voice clips — cloning it per call (each call re-conditions independently, so voices never bleed between videos), with a gentle, deterministic denoise on the output; three public-domain preset voices ship under `assets/voices/`. **Per-video description:** a workflow can attach a description (e.g. its sources) to each video, shown read-only in the run view (this is where the SSN workflow will list its sources). No workflow uses these yet — the Sensational Science News workflow itself is the next stage.
