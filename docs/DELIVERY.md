...
- **Environment:** an isolated venv under the install folder, built **from `requirements.txt`** (the pinned runtime manifest — there is no separate lockfile; the `==` pins are the lock, recorded in `PROJECT_STATUS.md`), with the SDK installed into it. The React frontend is **built by the installer** (`npm run build` → `app/web/`) and copied in as static files, so **Node is an install-time prerequisite but not a runtime one** — the running server serves the pre-built static assets and never invokes Node. (SFVF is owner-installed on a dev machine that already has Node; a future maintainer-packaged deliverable could pre-build and drop the Node install-time requirement.)
- **Prerequisites the installer checks (and names how to get, via `winget`, if missing):** Python 3.12.4+ (the `>=3.12.4` SDK security floor), **ffmpeg + ffprobe on PATH** (required by `sfvf._ffmpeg` for finalize/edit), and **Node/npm** (to build the frontend at install time). The installer refuses with a plain message naming any missing item rather than a half-install.
