...
provenance; do your `ctx.library.put(...)` in `prepare()` with your manifest-declared facets, and call
`ctx.library.find(...)` before sourcing to reuse a past selection for free. Downloaded bytes are
