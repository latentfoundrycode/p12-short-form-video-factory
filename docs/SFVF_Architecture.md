...
| HyperFrames | Local, no authentication | An existing external project with its own repository, integrated rather than built here — obtained from source and reached through an adapter. Requires a headless Chromium browser and installed fonts. |

**Kinocut and HyperFrames are existing external repositories, not components this project builds.** Each is obtained from its own source — cloned or vendored and installed — and reached through an adapter here, exactly as the API providers are reached through theirs. "Local, no authentication" describes how they run once present; it does not mean they originate in this codebase. Before either adapter is written its repository is investigated at source, to establish the client surface it exposes and the runtime it needs — for HyperFrames, the headless Chromium browser and installed fonts noted above. Treating them as build-from-scratch work both overstates the effort and misplaces it: the work is adapter-and-plumbing, and it belongs in the providers stage of the build order (§7) with the rest, not earlier.
